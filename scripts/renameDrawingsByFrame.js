...
function renameDrawingsByFrame() {
     ...
     */
    this.renameDrawings = function (inNode) {
        var NEW_PREFIX = "tmprename_";

        var col = node.linkedColumn(inNode, "drawing.element");
        if (!col) {
        ...
        for (var f = 1; f < frame.numberOf() + 1; f += 1) {
            var drawingName = column.getEntry(col, 1, f);
            ...
            if (
                ...
                var drawingNameTemp = NEW_PREFIX + f;
        ...
        // Rename to final
        column.getDrawingTimings(col).forEach(function (x) {
